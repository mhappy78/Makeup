...
class FaceSimulator:
    ...
    def setup_ui(self):
        ...
        # 탭 노트북 (고정 높이로 제한)
        notebook_frame = ttk.Frame(control_frame, height=500)
        ...
        self.notebook.add(self.warp_frame, text="🎨 자유 변형")
            ...
            for group_name, group_data in landmark_groups.items():
                ...
                for idx in indices:
                    if idx < len(self.face_landmarks.landmark):
                        ...
                        # 점 크기 (사용자 설정 + 줌 고려)
                        point_size = max(1, int(self.landmark_point_size * self.zoom_factor))
